...
# Use toml as new config file format
# by branchvan
def set_conf():
    conf = toml.load("./config.toml")
    MasterService.exposed_Master.block_size = int(conf["master"]["block_size"])
    MasterService.exposed_Master.replication_factor = int(conf["master"]["replication_factor"])
    minions_list = conf["minions"]["list"]
    for m in minions_list:
        id, host, port = m.split(":")
        MasterService.exposed_Master.minions[id] = (host, port)

    if os.path.islink("fs.img"):
        MasterService.exposed_Master.file_table, MasterService.exposed_Master.block_mapping = pickle.load(
                     open('fs.img', 'rb'))
